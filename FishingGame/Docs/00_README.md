...
> 최종 갱신: 2026-09-04 (After-Bite + arrival/respawn validation)
> 상태: **코어 루프 확정 · unified water composite/visible surface optics 튜닝 검증 · v25 데이터/이동 이식 · Strike/Hooked/mouth contact/자동회수/projected shadow/After-Bite/도착 집계 통합 완료 · After-Bite별 시각 밸런스 확인 완료 · 정상 회귀 캡처 완료 · procedural 5종 아트 기준 확정 · 포폴 HUD 아트 이식 완료** (§12 로드맵 참조)
>
...
| 파일 | 내용 | 언제 보나 |
|---|---|---|
| **01_기획서_v0.4.md** | 기획의 단일 원본. 설계 원칙 · 코어 루프 · 상태 머신 · 실제 어종 5종 실수치 · 데이터 모델 · Unity 구조 · MVP 범위 | 구현 전, 그리고 규칙이 헷갈릴 때마다 |
...
현재 Unity 작업 순서는 `Docs/06_v25_이관상태_v0.1.md`의 체크리스트를 단일 기준으로 삼는다.
물 표현 브랜치 `codex/water-presentation`의 `5bd0f12`는 현재 행동 브랜치에 머지되어 있으며,
물쪽 프로파일·수중 카메라·ripple/shader 변경을 유지한 채 다음 순서로 진행한다.

1. procedural 5종 생성 출력의 최종 시각 검수와 필요시 VisualSpec 미세 조정
2. v25 전체 어종 90초 게이트와 After-Bite/정상 회귀 증거를 유지한 채 사운드 → UI/세이브로 확장
3. 낚시터 2곳을 추가

병행: 02 §12-1/12-2로 **가로·세로 컨셉** 뽑아 방향 결정, 실제 어종 5종 개별 생성
